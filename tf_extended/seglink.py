...
def match_anchor_to_text_boxes_fast(anchors, xs, ys):
    ...
    attention_label = np.zeros(config.image_shape, dtype=np.int32)
    for bbox_idx in range(num_bboxes):
        bbox_points = zip(xs[bbox_idx, :], ys[bbox_idx, :])
    ...
    attention_label[points_in_bbox_mask] = 1  # 加入attention mask
    label_mask = attention_label.copy()  # 加入label mask, 用于还原原始标签
    ...
    return seg_labels, seg_locations, attention_label[::4, ::4, np.newaxis].astype(np.float32)
...
def generate_new_anchors(seg_offsets_pred):
    anchors = config.default_anchors
    anchor_cx, anchor_cy, anchor_w, anchor_h = (anchors[:, idx] for idx in range(4))
    ...
    seg_loc = np.transpose(np.vstack([seg_cx, seg_cy, seg_w, seg_h]))
    return seg_loc


def get_refine_seglink_gt(xs, ys, ignored):
    ...
    # 提取有效坐标，用于兼容refine stage
    anchors = config.refine_anchors
    refine_valid_index = np.where(ignored != -1)
    ignored = ignored[refine_valid_index]
    xs = xs[refine_valid_index]
    ys = ys[refine_valid_index]

    seg_labels, seg_locations, attention_labels = match_anchor_to_text_boxes_fast(anchors, xs, ys)
    ...
    return seg_labels, seg_offsets, link_labels, attention_labels,
...
def get_all_seglink_gt(xs, ys, ignored):
    ...
    seg_offsets = np.asarray(seg_offsets, dtype=np.float32)
    link_labels = np.asarray(link_labels, dtype=np.int32)
    # 备份原始label坐标，用于refine阶段
    bk_ignored = np.ones(1000) * -1
    bk_xs = np.ones((1000, 4)) * -1
    bk_ys = np.ones((1000, 4)) * -1
    valid_index = np.where(ignored != -100)
    bk_ignored[valid_index] = ignored
    bk_xs[valid_index] = xs
    bk_ys[valid_index] = ys

    return seg_labels, seg_offsets, link_labels, attention_labels, \
           bk_xs.astype(np.float32), bk_ys.astype(np.float32), bk_ignored.astype(np.int32)
...
def tf_get_all_seglink_gt(xs, ys, ignored):
    ...
    xs = xs * w_I
    ys = ys * h_I
    seg_labels, seg_offsets, link_labels, attention_labels, gxs, gys, gignored = tf.py_func(get_all_seglink_gt,
                                                                                            [xs, ys, ignored],
                                                                                            [tf.int32, tf.float32,
                                                                                             tf.int32, tf.float32,
    ...
    attention_labels.set_shape([h_I / 4, w_I / 4, 1])
    gxs.set_shape([1000, 4])
    gys.set_shape([1000, 4])
    gignored.set_shape([1000, ])

    return seg_labels, seg_offsets, link_labels, attention_labels, gxs, gys, gignored
...
def seglink_to_bbox(seg_scores, link_scores, seg_offsets_pred,
    ...
    for group_index in seg_groups:
        group = [seg_locs[idx, :] for idx in group_index]
        scores_group = [seg_scores[idx] for idx in group_index]
        bbox_score = np.asarray(sum(scores_group) / len(scores_group))
...
def bboxes_to_xys(bboxes, image_shape):
    ...
    assert np.ndim(bboxes) == 2 and np.shape(bboxes)[-1] == 6, 'invalid `bboxes` param with shape =  ' + str(
        np.shape(bboxes))
    ...
    for bbox_idx, bbox in enumerate(bboxes):
        ...
        xys[bbox_idx, :8] = points
        xys[bbox_idx, 8] = score
    return xys